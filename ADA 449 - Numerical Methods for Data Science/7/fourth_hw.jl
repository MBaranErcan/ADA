...
function constraints(x::AbstractVector)::Tuple{Float64, Float64}
    ...
    ## one is the function of the constraint norm(x) ≤ 3.7
...
function unit_test_1()::Bool
    ...
    ##
    @info "OK Computer!!!" ### BEST ALBUM EVER!!!! ################################################################################
    return 1
end
...
function minimize(objective::Function, 
    ...
    for i in 1:max_iter
        grad = Zygote.gradient(x->objective(x) + λ*constraints(x)[1] + μ*constraints(x)[2], x_init)[1]
