...
function derivative(f::Function, x::Real; h::Float64 = 1e-5)::Float64
    return (f(x + h) - f(x)) / h    #(f(x+h) -f(x-h))/(2h)
end
...
function find_minimum_bisection(f::Function, a::Real, b::Real; max_iter::Int = 100, ϵ::Float64 = 1e-5)::Tuple{Float64, Float64}
    ...
    for k in 1:max_iter
        α = (a + b) / 2 # midvalue
        f_ = derivative(f, α)   # first derivative of f at α
        if  f_ < 0
            a = α
        else
            b = α
        ...
        if abs(a- b) < ϵ
            @info "Converged"
            println("iterations: $(k)") # Number of iterations
            return α, f(α)
        end
    end
...
function unit_test_for_bisection()
    @assert isa(find_minimum_bisection(x->x^2 , -1, 1), Tuple{Float64, Float64}) "Return type should be a tuple of Float64s"
    try
